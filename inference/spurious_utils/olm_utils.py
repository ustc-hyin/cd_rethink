...
def greedy_search(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        use_cd = model_kwargs.get("use_olm") != None
        ...
        if use_cd:
            ...
            # modify the output distribution
            sum_prob = probs[:,YES_INDEX]+probs[:,NO_INDEX]

            if sum_prob > 0.2:
                abs_diff = torch.abs(probs[:,YES_INDEX]-probs[:,NO_INDEX])
                if abs_diff < 0.5:
                    next_tokens = torch.tensor([YES_INDEX], device=probs.device) 
                else:
